...
def MATRIX_likelihood(Omega_r0, LIST_Omega_m0, LIST_Omega_Lambda0, c, H_0, redshifts, magnitudes, error_magnitudes):
    ...
    for j in range(len(LIST_Omega_Lambda0)):
        ...
        for i in range(len(LIST_Omega_m0)):
            ...
            print('***** computation time for likelihood: %s seconds *****'%(time.time() - start_time))
            ROW.append(L)
        MATRIX.append(ROW)
    return MATRIX
...
def find_best_fit_values(LIST_Omega_m0, LIST_Omega_Lambda0, MATRIX_likelihood):
    ...
    return Omega_m0_max_likelihood, Omega_Lambda0_max_likelihood
...
def main():    
    ...
    LIST_Omega_Lambda0 = np.arange(0.0, 1.5, 0.01)

    MATRIX_like = MATRIX_likelihood(Omega_r0, LIST_Omega_m0, LIST_Omega_Lambda0, c, H_0, redshifts, magnitudes, error_magnitudes)
    
    best_values = find_best_fit_values(LIST_Omega_m0, LIST_Omega_Lambda0, MATRIX_like)
    
    print('********** TOTAL COMPUTATION TIME: %s seconds **********'%(time.time() - START_TOTAL_TIME))
    ...
    plt.title('best fit values: $(\Omega_{m,0}, \Omega_{\Lambda,0}) = (%.2f, %.2f)$'%(best_values))
